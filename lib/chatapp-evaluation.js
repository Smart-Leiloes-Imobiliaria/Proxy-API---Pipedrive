...
const KNOWN_ASSESSOR_NAMES = {
  "analise de credito - yara": "Análise de crédito - Yara"
};
...
function authorOf_(message) {
  ...
  const explicitAssessorName = explicitAssessorNameFromText_(messageText_(message));
  const rawName = explicitAssessorName &&
    isBlockedInternalAssessor_({ id: sender && sender.id, name: senderName, identifiers: [createdId] })
    ? explicitAssessorName
    : senderName;
...
function explicitAssessorNameFromText_(text) {
  const match = cleanText_(text).match(/^\*([^*\r\n]{2,100})\*/);
  return match ? KNOWN_ASSESSOR_NAMES[normalizeName_(match[1])] || "" : "";
}
