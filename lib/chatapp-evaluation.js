...
const BLOCKED_INTERNAL_ASSESSOR_IDENTIFIERS = new Set(["5531973239098", "66345"]);
...
async function handleEvaluation(input, dependencies) {
  ...
  try {
    ...
    if (summaryHasBlockedInternalAssessor_(transcriptSummary)) return notEvaluable_(metadata, "blocked_assessor");
    const candidates = filterBlockedEvaluationCandidates_(transcriptSelection.candidates);
...
function summaryHasBlockedInternalAssessor_(summary) {
  if (!blockInternalAssessorEvaluation_()) return false;
  const participants = Array.isArray(summary && summary.participants) ? summary.participants : [];
  if (participants.some(isBlockedInternalAssessor_)) return true;
  return (Array.isArray(summary && summary.lines) ? summary.lines : [])
    .some((line) => line && typeof line === "object" && line.side === "out" && isBlockedInternalAssessor_(line.author));
}

function candidateIdentifiers_(candidate) {
  const values = [candidate && candidate.id, candidate && candidate.name].concat(candidate && Array.isArray(candidate.identifiers) ? candidate.identifiers : []);
  const identifiers = [];
  for (const value of values) {
    const raw = cleanId_(value);
    if (!raw) continue;
...
function authorOf_(message) {
  ...
  const id = createdId || mappedId || senderId || null;
  const identifiers = [createdId, mappedId, senderId, message && message.created && message.created.creatorId]
    .map(cleanId_)
    .filter(Boolean);
  return { id: cleanId_(id), name: cleanText_(rawName), identifiers: Array.from(new Set(identifiers)) };
}
