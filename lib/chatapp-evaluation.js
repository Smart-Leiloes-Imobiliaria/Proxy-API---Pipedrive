...
async function handleEvaluation(input, dependencies) {
  ...
  const missing = ["chat_id", "license_id", "messenger_type", "closed_at"].find((key) => !request[key]);
  if (missing) return result_(400, { success: false, error: "missing_" + missing });
  if (!request.closedAt) return result_(400, { success: false, error: "invalid_closed_at" });
  ...
  try {
    const chat = await services.getChat(request);
    ...
    if (metadata.responsibleId) {
      const employee = unwrapData_(await services.getEmployee(metadata.responsibleId, request));
    ...
    const messages = await services.listMessages(request);
    const transcriptSummary = buildTranscriptSummary_(messages, {
      responsibleId: metadata.responsibleId,
      responsibleName,
      sessionStartedAt: request.sessionStartedAt,
      sessionClosedAt: request.closedAt
    });
    const candidates = evaluationCandidates_(transcriptSummary, { responsibleId: metadata.responsibleId, responsibleName });
    if (!candidates.length) return notEvaluable_(metadata, "no_human_messages");

    const hasEvaluationRecord = services.hasEvaluationRecord || services.hasRecord;
    if (await allCandidatesAlreadyProcessed_(hasEvaluationRecord, candidates, metadata)) {
      return result_(200, { success: true, status: "already_processed", chat_id: metadata.chatId });
    }

    const saved = [];
    for (const candidate of candidates) {
      if (await hasEvaluationRecord(metadata.chatId, metadata.closedAtDisplay, candidate.name)) continue;
      ...
      // Reconfere imediatamente antes da escrita: protege retries sequenciais e
      // concorrencias que terminem enquanto a chamada ao modelo esta em curso.
      if (await hasEvaluationRecord(metadata.chatId, metadata.closedAtDisplay, candidate.name)) continue;
      await services.appendRecord({
      ...
      saved.push({ responsibleName: candidate.name, nota: evaluation.nota, messages: transcript.targetHumanMessages });
    }
    ...
    return result_(200, {
      ...
      responsible_name: saved[0].responsibleName,
      evaluated_employees: saved.map((item) => ({ responsible_name: item.responsibleName, nota: item.nota })),
...
function buildTranscriptSummary_(messages, options) {
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    ...
    if (isExcluded_(message)) continue;
    ...
    if (side === "in") {
      lines.push("[" + time + "] CLIENTE: " + sanitizePii_(text));
      continue;
    }
    ...
    const participant = participants.get(key) || { id: author.id, name, count: 0 };
    participant.count += 1;
    participants.set(key, participant);
    lines.push({ time, side: "out", author: { id: author.id, name }, text: sanitizePii_(text) });
  }
...
function evaluationCandidates_(summary, options) {
  ...
  if (responsibleParticipant) selected.push({ id: responsibleParticipant.id || responsible.id, name: responsible.name || responsibleParticipant.name, count: responsibleParticipant.count });
  for (const participant of participants) {
    ...
    selected.push({ id: participant.id, name: participant.name, count: participant.count });
  }
  return selected.filter((item) => item.name && item.count > 0);
}

async function allCandidatesAlreadyProcessed_(hasEvaluationRecord, candidates, metadata) {
  for (const candidate of candidates) {
    if (!await hasEvaluationRecord(metadata.chatId, metadata.closedAtDisplay, candidate.name)) return false;
  }
  return candidates.length > 0;
}

function normalizeRequest_(body) {
  const closedDate = offsetChatAppDate_(toDate_(body.closed_at));
  ...
  const startedDate = offsetChatAppDate_(toDate_(body.session_started_at || body.started_at));
...
function toDate_(value) { const date = new Date(value); return Number.isNaN(date.getTime()) ? null : date; }
...
function log_(event, details) { console.log(JSON.stringify(Object.assign({ event, feature: "chatapp_evaluation" }, details || {}))); }
...
function authorOf_(message) {
  ...
  const rawName = (sender && (sender.fullName || sender.name)) || (message && message.created && message.created.name) || "";

  // Hardcoded map of known atendentes (normalized name -> responsible id).
  const nameToId = {
    "evelycastro.smart@gmail.com": "101592",
    "análise de crédito - yara": "99955",
  ...
  const mappedId = nameToId[normalizeName_(rawName)];
  // Priority: createdId (ChatApp employee id) -> mappedId (manual) -> sender.id
  const id = createdId || mappedId || (sender && sender.id) || null;
  return { id: cleanId_(id), name: cleanText_(rawName) };
}
function isExcluded_(message) {
  ...
  return !text || /^\//.test(text) || /system|debug|technical/i.test(kind) ||
    !!(message && (message.system || message.isSystem || message.automated || message.isBot)) ||
    AUTOMATION_PATTERN.test(sender.name) || AUTOMATION_PATTERN.test(String(message && message.fromApp && message.fromApp.name || ""));
}
...
module.exports = { handleEvaluation, buildTranscript, formatClosedAt_ };
