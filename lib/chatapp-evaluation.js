...
async function handleEvaluation(input, dependencies) {
  ...
  if (!closedAtDate) return result_(400, { success: false, error: "invalid_closed_at" });
  ...
  try {
    ...
    const transcriptSummary = transcriptSelection.summary;
    const candidates = transcriptSelection.candidates;
...
function transcriptWindows_(options) {
  const windows = [];
  const rawStartedAt = toDate_(options && options.rawSessionStartedAt);
  const rawClosedAt = toDate_(options && options.rawSessionClosedAt);
...
function normalizeRequest_(body) {
  ...
  if (debugEnabled_()) safeLog({ event: 'normalized_request', raw_closed_at: body.closed_at, closedAt: closedAtIso, raw_started_at: body.session_started_at || body.started_at, startedAt: startedAtIso });
  return {
    ...
    sessionStartedAtRaw: cleanText_(body.session_started_at || body.started_at)
  };
}
...
function authorOf_(message) {
  ...
  const rawName = (sender && (sender.fullName || sender.name)) ||
