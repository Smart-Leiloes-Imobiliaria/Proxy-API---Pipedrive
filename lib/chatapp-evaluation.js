...
async function handleEvaluation(input, dependencies) {
  ...
  try {
    ...
    const metadata = {
      ...
      chatLink: chatAppChatLink_(request),
    ...
    let transcriptSummary = filterBlockedEvaluationSummary_(transcriptSelection.summary);
    transcriptSummary = await resolveSummaryParticipantNames_(transcriptSummary, services, request);
...
async function resolveSummaryParticipantNames_(summary, services, request) {
  if (!summary || !services || typeof services.getEmployee !== "function") return summary;
  const participantIds = Array.from(new Set((summary.participants || [])
    .filter((participant) => shouldResolveParticipantName_(participant && participant.name))
    .map((participant) => cleanId_(participant && participant.id))
    .filter(Boolean)));
  if (!participantIds.length) return summary;

  const resolvedNames = new Map();
  await Promise.all(participantIds.map(async (id) => {
    try {
      const employee = unwrapData_(await services.getEmployee(id, request));
      const name = cleanText_(employee && (employee.fullName || employee.name));
      if (name) resolvedNames.set(id, name);
    } catch (_) {
      // Keep the ChatApp author name when employee lookup is unavailable.
    }
  }));

  if (!resolvedNames.size) return summary;
  const resolveAuthor = (author) => {
    const name = resolvedNames.get(cleanId_(author && author.id));
    return name ? Object.assign({}, author, { name }) : author;
  };
  return {
    lines: (summary.lines || []).map((line) => line && typeof line === "object" && line.side === "out"
      ? Object.assign({}, line, { author: resolveAuthor(line.author) })
      : line),
    participants: (summary.participants || []).map((participant) => {
      const name = resolvedNames.get(cleanId_(participant && participant.id));
      return name ? Object.assign({}, participant, { name }) : participant;
    }),
    hadReliableBoundary: !!summary.hadReliableBoundary
  };
}

function shouldResolveParticipantName_(name) {
  const raw = cleanText_(name);
  return !raw || raw === "[autor não identificado]" || !/[A-Za-zÀ-ÿ]/.test(raw);
}
...
function recordExistsInRows_(rows, chatId, closedAt, responsibleName) {
  return Array.isArray(rows) && rows.some((row) => String(row[0] || "") === String(chatId) && String(row[3] || "") === String(closedAt) && normalizeName_(row[2]) === normalizeName_(responsibleName));
}

function chatAppChatLink_(request) {
  const base = "https://cabinet.chatapp.online/businesses/v2/products/dialogs#/dialogs/";
  return base + [request.license_id, request.messenger_type, request.chat_id].map((value) => encodeURIComponent(String(value || ""))).join("/");
}
