...
function candidateIdentifiers_(candidate) {
  ...
  if (raw) {
    ...
    const nameDigits = cleanText_(candidate && candidate.name).replace(/\D/g, "");
    if (nameDigits) identifiers.push(nameDigits);
  }
...
function authorOf_(message) {
  ...
  return { id: cleanId_(id), name: cleanText_(rawName), identifiers: [cleanId_(id)].filter(Boolean) };
}
