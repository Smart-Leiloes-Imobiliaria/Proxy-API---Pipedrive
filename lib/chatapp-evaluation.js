...
function candidateIdentifiers_(candidate) {
  const values = [candidate && candidate.id].concat(candidate && Array.isArray(candidate.identifiers) ? candidate.identifiers : []);
  return Array.from(new Set(values.map(cleanId_).filter(Boolean)));
}
