...
function createBitrixClient(options) {
  ...
  async function callMethod(memberId, method, params) {
    ...
    if (installation) {
      const usableInstallation = tokenNeedsRefresh_(installation, config)
        ? await refresh_(installation)
        : installation;
      return callOAuth_(usableInstallation, method, params || {}, true);
    }
  ...
  async function refresh_(installation) {
    ...
    const clientId = clean_(config.clientId || process.env.BITRIX_CLIENT_ID);
    const clientSecret = clean_(config.clientSecret || process.env.BITRIX_CLIENT_SECRET);
    if (!clientId || !clientSecret) throw error_("bitrix_oauth_client_credentials_missing", 500);
...
function tokenNeedsRefresh_(installation, config) {
  const expiresAt = Number(installation && installation.expires_at);
  if (!Number.isFinite(expiresAt) || expiresAt <= 0) return false;
  const rawSkew = config.refreshSkewMs != null ? config.refreshSkewMs : process.env.BITRIX_OAUTH_REFRESH_SKEW_MS;
  const skewMs = Math.max(0, Number(rawSkew == null || rawSkew === "" ? 60000 : rawSkew));
  return expiresAt <= Date.now() + (Number.isFinite(skewMs) ? skewMs : 60000);
}
