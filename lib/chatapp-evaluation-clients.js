...
function createSheetsRepository_() {
  ...
  return {
    ...
    appendRecord: async (record) => {
      await sheetsRequest_(url + ":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS", "POST", { values: [[record.chatId, record.chatLink || "", record.responsibleName, record.closedAt, record.clientName, record.nota, record.justificativa || "", record.capturedTexts || "", record.source || ""]] });
    }
  };
}
