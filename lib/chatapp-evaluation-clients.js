...
function createChatAppClient_() {
  ...
  return {
    ...
    listMessages: async (data) => {
      ...
      const messages = []; let next = base + "?limit=100&includeSystemMessages=0";
      while (next) {
        const page = await request(next);
        const body = page && page.data ? page.data : page;
        messages.push(...(Array.isArray(body) ? body : body.messages || body.items || []));
...
async function evaluateWithOpenAI_(input) {
  ...
  const response = await fetchJson_("https://api.openai.com/v1/responses", {
    ...
    method: "POST",
    body: JSON.stringify({
      model: requiredEnv_("OPENAI_MODEL"), store: false, max_output_tokens: 180,
      instructions: prompt,
        ...
        format: {
          ...
          schema: {
            ...
            properties: {
              ...
              justificativa: { type: "string", maxLength: 280 }
            }
          }
        }
      }
  ...
  // Trim justificativa to a short form
  parsed.justificativa = String(parsed.justificativa || "").trim().replace(/\s+/g, " ").slice(0, 280);
  return parsed;
}
...
async function googleAccessToken_() {
  ...
  const assertion = header + "." + claim + "." + signer.sign(requiredEnv_("GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY").replace(/\\n/g, "\n")).toString("base64url");
...
async function fetchJson_(url, headers, service, options) {
  let response;
  try { response = await fetch(url, Object.assign({ headers, signal: AbortSignal.timeout(8000) }, options || {})); }
