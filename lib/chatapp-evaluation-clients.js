  ...
  return {
    ...
    appendRecords: async (records) => {
      ...
      await sheetsRequest_(url + ":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS", "POST", {
        values: rows.map((record) => [record.chatId, record.responsibleName, record.closedAt, record.clientName, record.nota, record.justificativa || "", record.capturedTexts || ""])
      });
    },
    appendRecord: async (record) => {
      await sheetsRequest_(url + ":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS", "POST", { values: [[record.chatId, record.responsibleName, record.closedAt, record.clientName, record.nota, record.justificativa || "", record.capturedTexts || ""]] });
    }
  };
}
