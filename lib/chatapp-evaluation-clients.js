...
const MAX_MESSAGE_PAGES = Number(process.env.CHATAPP_MAX_MESSAGE_PAGES) || 50;
    ...
    listMessages: async (data) => {
      ...
      while (next) {
        ...
        if (pageCount >= MAX_MESSAGE_PAGES) {
          safeLog({ event: "max_message_pages_reached", pageCount, base });
          break;
        }
      }
      return messages;
...
async function evaluateWithOpenAI_(input, options) {
  const opt = options || {};
  const prompt = evaluationPrompt_(opt);
  const transcriptText = String(input && input.transcript || "").trim();
  const transcriptLimit = Number(process.env[opt.transcriptLimitEnv || "CHATAPP_EVAL_MAX_TRANSCRIPT_CHARS"] || opt.defaultTranscriptLimit || "4000");
  ...
  if (String(process.env[opt.debugEnv || "DEBUG_CHATAPP_EVAL"] || "").trim()) {
    try {
...
function createSheetsRepository_() {
  ...
  const appendRange = encodeURIComponent("'" + sheetName.replace(/'/g, "''") + "'!A:A");
  const appendUrl = "https://sheets.googleapis.com/v4/spreadsheets/" + encodeURIComponent(spreadsheetId) + "/values/" + appendRange;
  return {
    ...
    hasRecord: async (chatId, closedAt) => {
      ...
      return values.some((row) => String(row[0] || "") === String(chatId) && String(row[3] || "") === String(closedAt));
    },
    hasEvaluationRecord: async (chatId, closedAt, responsibleName) => {
      const values = (await sheetsRequest_(url, "GET")).values || [];
      return values.some((row) => String(row[0] || "") === String(chatId) && String(row[3] || "") === String(closedAt) && normalizeName_(row[2]) === normalizeName_(responsibleName));
    },
    appendRecords: async (records) => {
      const rows = Array.isArray(records) ? records : [];
      if (!rows.length) return;
      await sheetsRequest_(appendUrl + ":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS", "POST", {
        values: rows.map((record) => [record.chatId, record.chatLink || "", record.responsibleName, record.closedAt, record.clientName, record.nota, record.justificativa || "", record.capturedTexts || "", record.source || ""])
      });
    },
    appendRecord: async (record) => {
      await sheetsRequest_(appendUrl + ":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS", "POST", { values: [[record.chatId, record.chatLink || "", record.responsibleName, record.closedAt, record.clientName, record.nota, record.justificativa || "", record.capturedTexts || "", record.source || ""]] });
    }
  };
}
...
function evaluationPrompt_(options) {
  const opt = options || {};
  const filePath = env_(opt.promptFileEnv || "EVALUATION_PROMPT_FILE") || (opt.fallbackPromptFileEnv ? env_(opt.fallbackPromptFileEnv) : "");
  if (filePath) {
    const tried = [];
    ...
    try {
      ...
      tried.push(String(err && err.path || ""));
      // try relative to this module (safer for serverless bundles)
      try {
        const altPath = path.resolve(__dirname, "..", filePath);
        tried.push(altPath);
        const value = fs.readFileSync(altPath, "utf8").trim();
        if (!value) throw externalError_("missing_evaluation_prompt_file_content", 500);
        return value;
      } catch (err2) {
        // fallback to bundled JS module (ensures prompt is available in deploy)
        try {
          const bundled = require("./evaluation-prompt");
          if (bundled) return bundled;
        } catch (_) {}
        // fallback to env variable with the prompt content
        const envPrompt = env_("EVALUATION_PROMPT");
        if (envPrompt) return envPrompt;
        // no prompt available
        throw externalError_("missing_evaluation_prompt_file:" + tried.join(","), 500);
      }
    }
  }
  return requiredEnv_(opt.promptEnv || "EVALUATION_PROMPT");
}
...
module.exports = { createServices, evaluateWithOpenAI: evaluateWithOpenAI_, sheetsRequest: sheetsRequest_ };
