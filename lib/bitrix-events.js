...
async function handleBitrixEvent(input, dependencies) {
  ...
  try {
    ...
    const eventId = positiveInt_(body.eventId || body.event_id || body.event_handler_id);
    ...
    logEventResult_({ eventId, accepted, duplicates, ignored });
...
function logEventResult_(summary) {
  try {
    console.info(JSON.stringify({
      event: "bitrix_on_session_finish",
      event_id: positiveInt_(summary && summary.eventId),
      accepted: positiveInt_(summary && summary.accepted),
      duplicates: positiveInt_(summary && summary.duplicates),
      ignored: positiveInt_(summary && summary.ignored)
    }));
  } catch (_) {}
}
