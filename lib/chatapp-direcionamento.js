...
const FORMA_PAGAMENTO_FINANCIADO_ID = "34";
...
function avaliarDealParaDirecionamento(deal) {
  const esteiraExistente = classificarProcessoDirecionamento_(deal);
  if (!esteiraExistente) {
    return { elegivel: false, delegacao: semDelegacao_("deal_fora_das_esteiras_de_direcionamento") };
  }

  const grupoEtapa = classificarGrupoEtapa_(deal);
  const delegacao = decidirDelegacao(deal, esteiraExistente, grupoEtapa);
  ...
  return {
    ...
    processo: esteiraExistente.processo,
...
function prioridadeAvaliacao_(delegacao, grupoEtapa, esteiraExistente) {
  if (esteiraExistente && esteiraExistente.key === "pos_arrematacao") return 0;
...
function decidirDelegacao(deal, esteiraExistente, grupoEtapa) {
  const esteira = esteiraExistente || classificarProcessoDirecionamento_(deal);
  const processoKey = esteira && esteira.key;

  if (processoKey === "pos_arrematacao") {
    return decidirDelegacaoPosArrematacao_(deal);
  }

  if (processoKey !== "analise_credito") {
    return semDelegacao_("sem_regra_de_direcionamento_para_esteira");
  }

  return decidirDelegacaoAnaliseCredito_(deal, grupoEtapa || classificarGrupoEtapa_(deal));
}

function decidirDelegacaoPosArrematacao_(deal) {
  ...
  return semDelegacao_("pos_arrematacao_sem_atribuido_financiamento_valido");
}

function decidirDelegacaoAnaliseCredito_(deal, grupoEtapa) {
  ...
  if (grupoEtapa && grupoEtapa.key === "analise_credito") {
    const analiseCredito = campoPessoa_(deal, "atribuido_analise_credito");
    if (
      campoBate_(analiseCredito, CUSTOM_OPTION_IDS.atribuido_analise_credito.daniela, ["daniela"]) ||
      ownerBate_(deal, "daniela")
    ) {
      return fixedDelegacao_(
        "ANALISE_CREDITO_DANIELA",
        CHATAPP_USERS.daniela,
        "atribuido_ou_owner_analise_credito_daniela"
      );
    }
    if (
      campoBate_(analiseCredito, CUSTOM_OPTION_IDS.atribuido_analise_credito.thales, ["thales"]) ||
      ownerBate_(deal, "thales")
    ) {
      return fixedDelegacao_(
        "ANALISE_CREDITO_THALES",
        CHATAPP_USERS.thales,
        "atribuido_ou_owner_analise_credito_thales"
      );
    }
    if (ownerBate_(deal, "yasmin")) return semDelegacao_("analise_credito_yasmin_sem_chatapp_user_id");
    if (ownerBate_(deal, "guilherme")) return semDelegacao_("analise_credito_guilherme_sem_chatapp_user_id");
    return semDelegacao_("analise_credito_owner_nao_reconhecido");
  }

  return semDelegacao_("analise_credito_etapa_nao_mapeada");
}
...
function classificarProcessoDirecionamento_(deal) {
  const pipelineId = pipelineId_(deal);
  const formaPagamentoId = String((deal && deal.forma_pagamento_id) || "");

  if (pipelineId === PIPELINE_POS_ARREMATACAO_ID && formaPagamentoId === FORMA_PAGAMENTO_FINANCIADO_ID) {
    return PROCESSOS.pos_arrematacao;
  }
  if (pipelineId === PIPELINE_ANALISE_CREDITO_ID) {
    return PROCESSOS.analise_credito;
  }
  return null;
}
