...
// Busca interna usada pela rota do ChatApp. Reusa o mesmo pipeline de
// /api/v2/deals/search e devolve apenas os deals "lean" (sem payload bruto).
async function internalSearchDeals_(term) {
  const payload = await handleDealsSearch_({
    method: "GET",
    path: "/api/v2/deals/search",
    query: { term: term, exact_match: "true", fields: "custom_fields,title" },
    headers: {}
  });
