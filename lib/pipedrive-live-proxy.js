...
function buildFilterDealPayload_(deal, lookups) {
  ...
  function pickField(key) {
    ...
    const val = entry.normalized_value !== undefined ? entry.normalized_value : entry.display_value;
  ...
  return compactObject_({
    ...
    criado_em: deal.add_time || null,
    etapa: compactObject_({
      id: deal.stage_id == null ? undefined : deal.stage_id,
      nome: stageName || undefined
    }),
    campos: compactObject_({
      "Estado": pickField(FILTER_21468_FIELD_KEYS.estado),
      "Cidade": pickField(FILTER_21468_FIELD_KEYS.cidade),
      "Número do Imóvel": pickField(FILTER_21468_FIELD_KEYS.numero_imovel),
      "Link Google Drive": pickField(FILTER_21468_FIELD_KEYS.link_google_drive),
      "Desconto % (Avaliação Smart x Pago)": pickField(FILTER_21468_FIELD_KEYS.desconto_smart_x_pago),
      "Valor: Total da Proposta": pickField(FILTER_21468_FIELD_KEYS.valor_total_proposta),
      "Valor médio estimado de venda": pickField(FILTER_21468_FIELD_KEYS.valor_medio_estimado_venda),
      "Valor avaliação Caixa": pickField(FILTER_21468_FIELD_KEYS.valor_avaliacao_caixa)
    })
  });
}
