...
const ROUTES = [
  ...
  "/api/chatapp/verificar-triagem-funis",
  "/api/chatapp/rotear-funil",
  "/api/chatapp/avaliar-atendimento",
  "/api/bitrix/install",
  "/api/bitrix/events",
  "/api/bitrix/processar-fila",
  "/api/bitrix/avaliar-atendimento"
];
...
async function handleNodeRequest(req, res) {
  ...
  const response = await handleProxyRequest({
    ...
    url: url.toString(),
    headers: req.headers,
    body: String(req.method || "").toUpperCase() === "POST" ? await readNodeBody_(req) : ""
  });
...
async function dispatchProxyRequest_(input) {
  ...
  // Rota simplificada para os funis do ChatApp. Recebe somente o telefone e
  // devolve todos os deals correspondentes com os tres campos de atribuicao.
  if (req.path === "/api/chatapp/verificar-triagem-funis") {
    const funis = require("./chatapp-funis.js");
    const result = await funis.handleVerificarTriagemFunis(
      { query: req.query, headers: req.headers },
  ...
  // Rota NOVA de roteamento persistente por funil de processo (id_chat + tipo).
  // Criada para nao alterar as rotas ChatApp ja existentes acima. Possui
  // autenticacao propria (CHATAPP_INTERNAL_TOKEN), tratada ANTES do
  // assertAuthorized_ do proxy. Erros tecnicos do searchDeals (timeout/
  // upstream) propagam propositalmente para o catch de handleProxyRequest,
  // que ja converte em 5xx/502/504 (ProxyHttpError) -- somente resultados de
  // negocio retornam 200 aqui.
  if (req.path === "/api/chatapp/rotear-funil") {
    const rotearFunil = require("./chatapp-rotear-funil.js");
    const result = await rotearFunil.handleRotearFunil(
      { query: req.query, headers: req.headers },
      internalSearchDeals_
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
  ...
  // Avaliacao secundaria do atendimento: possui autenticacao interna propria e
  // nunca cruza o fluxo somente-leitura do Pipedrive.
  if (req.path === "/api/chatapp/avaliar-atendimento") {
    const evaluation = require("./chatapp-evaluation.js");
    const clients = require("./chatapp-evaluation-clients.js");
    const result = await evaluation.handleEvaluation(
      { method: req.method, headers: req.headers, body: input.body },
      clients.createServices
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
  }

  if (req.path === "/api/bitrix/install") {
    const installation = require("./bitrix-installation.js");
    const result = await installation.handleBitrixInstallation(
      { method: req.method, headers: req.headers, body: input.body }
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
  }

  if (req.path === "/api/bitrix/events") {
    const events = require("./bitrix-events.js");
    const result = await events.handleBitrixEvent(
      { method: req.method, headers: req.headers, body: input.body }
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
  }

  if (req.path === "/api/bitrix/processar-fila") {
    const worker = require("./bitrix-queue-worker.js");
    const result = await worker.handleBitrixQueueWorker(
      { method: req.method, headers: req.headers, body: input.body }
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
  }

  // Avaliacao controlada do Bitrix24: consulta uma sessao Open Line por
  // SESSION_ID, com autenticacao e allowlist proprias.
  if (req.path === "/api/bitrix/avaliar-atendimento") {
    const evaluation = require("./bitrix-evaluation.js");
    const clients = require("./bitrix-evaluation-clients.js");
    const result = await evaluation.handleBitrixEvaluation(
      { method: req.method, headers: req.headers, body: input.body },
      clients.createServices
    );
    return { status: result.status || 200, cacheSeconds: 0, payload: result.payload };
...
async function readNodeBody_(req) {
  const chunks = [];
  let size = 0;
  for await (const chunk of req) {
    size += chunk.length;
    if (size > 64 * 1024) throw new ProxyHttpError("Request body too large.", "payload_too_large", 413);
    chunks.push(chunk);
  }
...
// Campos usados pelas rotas do ChatApp com direcionamento.
// Fonte: /home/davivieira/Downloads/data_fields_2026-07-08.xls.
const CHATAPP_DIRECIONAMENTO_FIELD_KEYS = {
  ...
  atribuido_triagem_documentos: "92c12e9a1b7987c5961b996eac85f462c4546f1a"
};
...
function extractOptionIds_(rawValue) {
  if (rawValue == null || rawValue === "") return [];
  ...
  for (let value of values) {
    if (value && typeof value === "object") {
      value = value.id != null ? value.id : (value.value != null ? value.value : null);
    }
...
function extractTextCustomField_(rawValue) {
  if (rawValue == null || rawValue === "") return "";
  if (rawValue && typeof rawValue === "object" && !Array.isArray(rawValue)) {
    if (rawValue.value != null) return String(rawValue.value);
    if (rawValue.label != null) return String(rawValue.label);
    if (rawValue.name != null) return String(rawValue.name);
    return "";
  }
  if (Array.isArray(rawValue)) return rawValue.map(extractTextCustomField_).filter(Boolean).join(", ");
...
const CHATAPP_DIRECIONAMENTO_OPTION_LABELS = {
  atribuido_triagem_documentos: {
    "2672": "Daniela Silva | Yasmin Rodrigues",
    "2760": "Daniela Silva | Isadora Campos",
    "2806": "Yasmin Rodrigues | Guilherme Oliveira"
  },
...
async function getLeanDealForTriagem_(dealId) {
  ...
  const triagemDocumentosRaw = cf[CHATAPP_DIRECIONAMENTO_FIELD_KEYS.atribuido_triagem_documentos];

  return {
    id: d.id,
    title: d.title || "",
    numero_imovel: extractTextCustomField_(cf[FILTER_21468_FIELD_KEYS.numero_imovel]),
    status: d.status || "", // status do ciclo de vida (open/won/lost)
    updated_at: d.update_time || d.updated_at || "",
    ...
    atribuido_triagem_documentos_ids: extractOptionIds_(triagemDocumentosRaw),
    atribuido_triagem_documentos: labelCustomOption_(triagemDocumentosRaw, CHATAPP_DIRECIONAMENTO_OPTION_LABELS.atribuido_triagem_documentos),
    atribuido_triagem_documentos_raw: triagemDocumentosRaw,
