...
async function fetchCandidatesForCriterion_(criterion, query) {
  ...
  if (criterion.kind === "property_code") {
    const candidates = await fetchDirectDealSearchCandidates_(baseQuery);
    return candidates.filter((candidate) => candidate && dealMatchesCriterion_(candidate.deal, criterion));
  }
...
function buildLeanPersonPayload_(person) {
  ...
  return compactObject_({
    ...
    cpf_cnpj: extractPersonCpfCnpjValue_(person, "cpf") || extractPersonCpfCnpjValue_(person, "cnpj") || undefined,
    fields: fields
  });
}
