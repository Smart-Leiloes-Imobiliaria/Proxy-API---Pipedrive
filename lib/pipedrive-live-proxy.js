...
// e lido direto do upstream a cada chamada) e sem buscar pessoa vinculada. O nome da
// etapa e resolvido pelo MAP hardcoded TRIAGEM_STAGE_NAMES (sem chamada a /stages). Apenas
// as opcoes de campos enum usam cache de 6h. Le telefone/status pelo hash.
async function getLeanDealForTriagem_(dealId) {
  const [json, fieldMaps] = await Promise.all([
