...
// e lido direto do upstream a cada chamada) e sem buscar pessoa vinculada. Apenas os
// metadados (stages + opcoes de campos) usam cache de 6h. Le telefone/status pelo hash
// e resolve etapa (nome do stage) e os labels dos campos enum via lookups em memoria.
async function getLeanDealForTriagem_(dealId) {
  const [json, stages, fieldMaps] = await Promise.all([
    pipedriveGetV2_("/deals/" + Number(dealId), {}), // fresco, sem cache de TTL
    getStagesMap_(),
    getDealFieldMaps_()
  ]);
