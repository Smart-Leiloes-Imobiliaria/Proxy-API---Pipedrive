...
// Busca interna ENXUTA usada pela rota do ChatApp. Diferente de handleDealsSearch_,
// NAO aciona a varredura de property_code nem o enriquecimento de pessoa: faz um unico
// /deals/search no upstream e, para cada deal casado, le apenas os campos necessarios
// (id, titulo, status, pipeline_id, stage_id e o telefone do arrematante pelo hash).
async function internalSearchDeals_(term) {
  const searchJson = await fetchReadonlyUpstreamJson_("/api/v2/deals/search", {
    term: term,
  ...
  const deals = await mapLimit_(uniqueNumbers_(ids), DEFAULT_CONFIG.concurrency, (id) => {
    return getLeanDealForTriagem_(id);
  });

  return deals.filter(Boolean);
}

// Campo customizado "status" da esteira Pos arrematacao (enum). O valor bruto e o id
// da opcao; o label e resolvido via getDealFieldMaps_ (cacheado).
const TRIAGEM_STATUS_FIELD_KEY = "888498f1de2185bbf6259cee93c859bf3088ff00";

// Mapa stage_id -> nome (cacheado 6h). Uma unica chamada a /stages.
async function getStagesMap_() {
  return getCached_("PD_TRIAGEM_STAGES_MAP_V2", DEFAULT_CONFIG.metadataCacheTtlSeconds, async () => {
    const arr = await fetchAllV2_("/stages", { limit: DEFAULT_CONFIG.upstreamMaxLimit });
    const map = {};
    for (const s of arr || []) {
      if (s && s.id != null) map[String(s.id)] = String(s.name || "");
    }
    return map;
  });
}

// Resolve o label de um valor de campo enum a partir de options_by_id (id -> label).
function resolveOptionLabel_(rawValue, fieldMeta) {
  ...
  const byId = (fieldMeta && fieldMeta.options_by_id) ? fieldMeta.options_by_id : {};
  const label = byId[String(id)];
  return (label != null && label !== "") ? label : String(id);
}

// Detalhe minimo do deal para a Triagem (cacheado). Sem buscar pessoa vinculada:
// le telefone/status direto pelo hash e resolve etapa (nome do stage) e o label do
// campo de status usando metadados cacheados (lookups em memoria, sem rede extra).
async function getLeanDealForTriagem_(dealId) {
  const cacheKey = ["pd_triagem", "deal_lean", String(dealId)].join(":");
  return getCached_(cacheKey, DEFAULT_CONFIG.detailCacheTtlSeconds, async () => {
    const [json, stages, fieldMaps] = await Promise.all([
      fetchReadonlyUpstreamJson_("/api/v2/deals/" + Number(dealId), {}),
      getStagesMap_(),
      getDealFieldMaps_()
    ]);
    ...
    const statusFieldMeta = fieldMaps && fieldMaps.byKey ? fieldMaps.byKey[TRIAGEM_STATUS_FIELD_KEY] : null;
    ...
    return {
      ...
      etapa: stageId != null ? (stages[String(stageId)] || "") : "",
      status_custom: resolveOptionLabel_(cf[TRIAGEM_STATUS_FIELD_KEY], statusFieldMeta),
      fields: { "Telefone do Arrematante": cf[FILTER_21468_FIELD_KEYS.telefone_arrematante] }
    };
  });
}
