...
async function buscarItensPorTelefone(variantes, searchDeals) {
  // Dedupe das variacoes numericas (a busca ignora o "+").
  const termos = Array.from(new Set(variantes.map((v) => v.replace(/\D+/g, "")).filter(Boolean)));

  // UMA chamada com todos os termos: o transporte faz as buscas em paralelo, junta os
  // candidatos unicos e busca cada deal uma unica vez (sempre fresco).
  const deals = (await searchDeals(termos)) || [];

  let matchesCount = 0;
  let melhor = null; // { deal, esteira }
  ...
  for (const deal of deals) {
    ...
    // So consideramos deals cuja Forma de Pagamento seja 34 (Financiado / FGTS Financiado).
    if (String(deal.forma_pagamento_id || "") !== "34") continue;
    ...
    if (!melhor || esteira.prioridade < melhor.esteira.prioridade) {
      melhor = { deal: deal, esteira: esteira };
    }
  }
...
// Telefone (so digitos) gravado no campo "Telefone do Arrematante" do deal.
// Usado como matched_phone / cliente.telefone no resultado.
...
function montarRespostaChatApp(resultado) {
  ...
  return {
    ...
    cliente: { telefone: "", processo: "", etapa: "", status: "", tipo_item: "", item_id: "", titulo: "", link: "" },
...
//   - pipeline_id != 6  -> "n/a" (nao e Pos arrematacao)
//   - pipeline_id == 6:
//       - Forma de Pagamento == 34 (Financiado / FGTS Financiado):
//           - Valor: FGTS > 0  -> Status: FGTS Financiado
//           - Valor: FGTS <= 0 -> Status: Financiamento
//       - outras formas de pagamento -> "" (sem status de financiamento definido)
// "n/a" significa estritamente "fora do Pos arrematacao".
