...
// O campo status so traz valor quando o deal esta no pipeline 6 (Pos arrematacao);
// em qualquer outro pipeline o valor e "n/a".
function montarStatus_(deal) {
  const pipelineId = deal.pipeline && deal.pipeline.id != null ? String(deal.pipeline.id) : "";
  if (pipelineId !== "6") return "n/a";
  return String(deal.status_custom || "") || "n/a";
}
