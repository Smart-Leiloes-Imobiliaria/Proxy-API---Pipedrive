...
function normalizeBitrixHistory(result, options) {
  ...
  const lines = Object.values(result.message).map((message) => {
...
function isSystemMessage_(message, senderId, user, rawText) {
  ...
  if (senderId === "0" && /\b(aceitou|entrou|saiu|foi transferid[oa])\s+(a|da|na)?\s*(conversa|diálogo|dialogo)\b/i.test(rawText)) return true;
